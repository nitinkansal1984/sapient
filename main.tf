...
resource "null_resource" "installansible" {
  ...
  provisioner "local-exec" {
      command = <<EOT
        ...
        ANSIBLE_HOST_KEY_CHECKING=False sudo ansible-playbook --user "demouser" -i '${module.network.publicip},' --private-key /tmp/mykey.pem  -e ansible_ssh_port=${each.value} apache.yml;
#        rm -rf /tmp/mykey.pem; 
    EOT
  }
}
